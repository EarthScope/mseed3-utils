...
void display_help(char *program_name, char *usage, char *message, const struct xseed_option_s *opts)
{
    printf("%s\n", message);
    printf("\nUsage: %s%s\n", program_name, usage);
    ...
    for (; opt_ptr != NULL && (0 != opt_ptr->short_option || 0 != opt_ptr->long_option || 0 != opt_ptr->description ||
                               0 != opt_ptr->variable_to_fill || 0 != opt_ptr->argument_type); opt_ptr++)
    {
