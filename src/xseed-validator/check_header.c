...
bool parse_header(struct warn_options_s *options, char *buffer, uint8_t *identifier_len, uint16_t *extra_header_len,
                     ...
                     uint32_t recordNum, int8_t verbose)
{
    ...
    //Run validation
    header_valid = parse_header(options, buffer, identifier_len, extra_header_len, payload_len, payload_fmt, recordNum,
                                verbose);
                  ...
                  uint32_t *payload_len, uint8_t *payload_fmt, uint32_t recordNum, uint8_t verbose)
{
    ...
    *payload_fmt = payload;
    ...
    {
        case 0:
            if (verbose > 1)printf("Payload flag indicates TEXT payload\n");
            break;
        case 1: /* 16-bit, integer, little-endian*/
            if (verbose > 1)printf("Payload flag indicates 16-bit, integer, little-endian payload\n");
            break;
        case 3: /* 32-bit, integer, little-endian*/
            if (verbose > 1)printf("Payload flag indicates 32-bit, integer, little-endian payload\n");
            break;
        case 4: /* IEEE 32-bit floats, little-endian */
            if (verbose > 1)printf("Payload flag indicates IEEE 32-bit floats, little-endian payload\n");
            break;
        case 5: /* IEEE 64-bit floats (double), little-endian */
            if (verbose > 1)printf("Payload flag indicates IEEE 64-bit floats (double), little-endian payload\n");
            break;
        case 10: /* Steim-1 integer compressin, big-endian */
            if (verbose > 1)printf("Steim-1 integer compression, big-endian\n");
            break;
        case 11: /* Steim-2 integer compressin, big-endian */
            if (verbose > 1)printf("Steim-2 integer compression, big-endian\n");
            break;
        ...
        case 16: /* GEOSCOPE Muxed 16/4 bit gain/exp ,little endian(?) */
            if (verbose > 1)printf("GEOSCOPE Muxed 16/4 bit gain/exp, little endian(?)\n");
            break;
        case 19: /* Steim-3 integer compressin, big-endian */
            if (verbose > 1)printf("Steim-3 integer compression, big-endian\n");
            break;
            //TODO verify this with chad
        case 30: /* SRO Gain Ranged Format, ?-endian */
            if (verbose > 1)printf("SRO Gain Ranged Format, ?-endian\n");
            break;
            //TODO verify this with chad
        case 32: /* DWWSSN Format, ?-endian */
            if (verbose > 1)printf("DWWSSN Format, ?-endian\n");
            break;
        case 53: /* 32-bit integer, little-endian, general compressor */
            if (verbose > 1)printf("32-bit integer, little-endian, general compressor\n");
            break;
        case 54: /* 32-bit IEEE floats, little-endian, general compressor */
            if (verbose > 1)printf("32-bit IEEE floats, little-endian, general compressor\n");
            break;
        case 55: /* 64-bit IEEE floats, little-endian, general compressor */
            if (verbose > 1)printf("64-bit IEEE floats, little-endian, general compressor\n");
            break;
...
#if 0
//Deprecated
bool parse_header_big_endian(struct warn_options_s *options,char *buffer, uint8_t *identifier_len, uint16_t *extra_header_len, uint32_t *payload_len, uint8_t *payload_fmt, uint8_t verbose)
{
   ...
   //Check Header flags
    ...
    h_flag[0] = (char)buffer[0];
    h_flag[1] = (char)buffer[1];
    printf( "Checking Header Flag value: %c%c\n", h_flag[0],h_flag[1]);
    if(!(h_flag[0] == 'M' && h_flag[1] == 'S'))
    {
        printf("Error: Header Flag Value Incorrect ('MS' is only valid flag)\n");
        header_valid = false;
        if(options->treat_as_errors)
        {
            return header_valid;
    ...
    //Check MS version
    uint8_t msVersion = (uint8_t)buffer[2];
    printf( "Checking File Version value: %d\n", msVersion);
    if (3 != msVersion)
    {
        printf("Error: Header Version Value Incorrect ('3' currently is the only supported MS version)\n");
        header_valid = false;
        if(options->treat_as_errors)
        {
            return header_valid;
    ...
    printf( "Checking Year value: %d\n", year);
    if(year < 0 || year > 65535)
    {
        printf("Error : Year value out of range (0-65535)\n");
        header_valid = false;
        if(options->treat_as_errors)
        {
            return header_valid;
    ...
    printf( "Checking Day of Year value: %d\n", doy);
    if (366 < doy || 1 > doy)
    {

        printf( "Error, Day Of Year value out of range (1-366)\n");
        header_valid = false;
        if(options->treat_as_errors)
        {
            return header_valid;
    ...
    */
    ...
    //Check valid hour range
    uint8_t hours = (uint8_t)buffer[12];
    printf( "Checking Hours value: %d\n", hours);
    if ( hours < 0 || hours > 23)
    {
        printf( "Error, Hours value out of range (0-23)\n");
        header_valid = false;
        if(options->treat_as_errors)
        {
            return header_valid;
    ...
    //Check valid min rage
    uint8_t mins = (uint8_t)buffer[13];
    printf( "Checking Mins value: %d\n", mins);
    if ( mins < 0 || mins > 59)
    {
        printf( "Error, Mins value out of range (0-59)\n");
        header_valid = false;
        if(options->treat_as_errors)
        {
            return header_valid;
    ...
    //Check valid seconds range
    uint8_t secs = (uint8_t)buffer[14];
    printf( "Checking Secs value: %d\n", secs);
    if ( secs < 0 || secs > 60 )
    {
        printf( "Error, Secs value out of range (1-366)\n");
        header_valid = false;
        if(options->treat_as_errors)
        {
            return header_valid;
    ...
    printf( "Checking Nanoseconds value: %d\n", nanoseconds);
    if (999999999 < nanoseconds)
    {
    ...
    uint8_t payload = (uint8_t)buffer[15];
    *payload_fmt = payload;
    printf( "Checking Payload Flag: %d\n", payload);
    switch (payload)
    {
        case 0:
        ...
        case 55: /* 64-bit IEEE floats, little-endian, general compressor */
            ...
            break;
        default:/* invalid payload type */
            printf( "Error: Payload Type Flag is Invalid!\n");
            header_valid = false;
            if(options->treat_as_errors)
            {
                return header_valid;
    ...
    printf( "Checking sample rate value: %f\n", sample_rate);
    ...
    printf( "Checking number of samples value: %d\n", number_samples);
