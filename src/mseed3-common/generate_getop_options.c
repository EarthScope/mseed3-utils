...
int
mseed3_get_short_getopt_string (char **short_opt_string, const struct mseed3_option_s *options)
{
  if (NULL == options)
  {
    return MSEED3_BAD_INPUT;
  }
  char *temp    = NULL;
  int total_len = 0, alloc_len = 0;
  for (const struct mseed3_option_s *option_ptr = options;
...
int
mseed3_get_long_getopt_array (struct option **long_opt_array, const struct mseed3_option_s *options)
{
  if (NULL == options)
  {
    return MSEED3_BAD_INPUT;
